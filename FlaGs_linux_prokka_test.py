...
myIMFont12 = ImageFont.truetype("Lato-Regular.ttf",size=12)
myIMFont7 = ImageFont.truetype("Lato-Regular.ttf",size=7)
					...
					if identicalProtID_Out!=query[0]: #can be anything XP_ or WP_ or YP_ or NP_
						...
						if not args.redundant:
							if special_out!='#':
								...
								#	queryDict[query[0]+'#'+str(q)]=asset
								#else:
								#	if query[0]!=special_out.split('|')[0]:
								#		queryDict[special_out.split('|')[0]+'#'+str(q)+'.'+query[0]]=asset
								#	else:
								#		queryDict[query[0]+'#'+str(q)]=asset
							else:
											...
											for line in lgffIn:											
												...
												if line.decode('utf-8')[0]!='#':
													...
													if Line[2]=='CDS':
														cds_c=1
														cset.add(cds_c)
														#if Line[8].split(';')[2][:5]=='Name=': #eliminates pseudo gene as they don't have 'Name='
														name_c=1
														nset.add(name_c)
														if Line[8].split(';')[0].split('=')[1]==query.split('#')[0]:
					...
					if line.decode('utf-8')[0]!='#':
						...
						if Line[2]=='CDS':
							...
							if query.split('_')[0]=='XP':
								...
								geneProt[Line[8].split(';')[1].split('=')[1]]=Line[8].split(';')[0].split('=')[1]
						...
						if Line[2][-4:]=='gene':
							...
							if query.split('_')[0]=='XP':
								...
								#for genDes in Line[8].split(';'):
								#	if 'gene_biotype=' in genDes:
								genDes="protein_coding"
								if Line[8].split(';')[0][3:] not in geneProt:
									geneProt[Line[8].split(';')[0][3:]]=genDes+'_'+query.split('#')[1]+'.'+str(random.randint(0,int(s)*2-1))+'*'
...
if args.tree:###Tree Command with ETE###
	...
	motifDict={}
	motifDict_2={}
	if glob.glob(args.out_prefix+'_tree/mafft_default-trimal01-none-fasttree_full/*fasta.final_tree.nw'):
		with open(glob.glob(args.out_prefix+'_tree/mafft_default-trimal01-none-fasttree_full/*fasta.final_tree.nw')[0], 'r') as treeIn:
			for line in treeIn:
				nwTree=line
		...
		print(args.out_prefix+'_tree/mafft_default-trimal01-none-fasttree_full/'+args.out_prefix+'_tree.fasta.final_tree.nw')
...
if args.tree and args.tree_order:  # Queries in postscript file will be presented as tree order
	...
	canvas.config(xscrollcommand=hbar.set, yscrollcommand=vbar.set)
	canvas.pack(side=LEFT,expand=True,fill=BOTH)
	image1 = Image.new("RGB", (widthM, heightM), 'white')
	...
	for eg in egs:
		if eg!='':
			...
			half_dom_height=5-round(postscriptSize(newQ))
			text = canvas.create_text(textspace/2-textspace/8,line_pos_y, text=org, fill="#404040", font=myFont12)
			draw.text((textspace/2-textspace/8,line_pos_y), org, fill="#404040", font=myIMFont12)
			for entry in entries:
				items=entry.split("\t")
				aln_start=round(int(items[5])*4/100)
				aln_end=round(int(items[6])*4/100)
				...
				if strandType=='+':
					rect = canvas.create_polygon(dom1_start, line_pos_y+half_dom_height, dom1_start, line_pos_y-half_dom_height,dom1_start+oL80, line_pos_y-half_dom_height, dom1_end, line_pos_y, dom1_start+oL80, line_pos_y+half_dom_height,fill=colorDict[dom1_name], outline=outliner(colorDict[dom1_name]))
					draw.polygon([dom1_start, line_pos_y+half_dom_height, dom1_start, line_pos_y-half_dom_height, dom1_start+oL80, line_pos_y-half_dom_height, dom1_end, line_pos_y, dom1_start+oL80, line_pos_y+half_dom_height],fill=colorDict[dom1_name], outline=outliner(colorDict[dom1_name]))
				else:
					rect = canvas.create_polygon(dom1_end-oL80, line_pos_y+half_dom_height, dom1_start, line_pos_y, dom1_end-oL80, line_pos_y-half_dom_height,dom1_end, line_pos_y-half_dom_height, dom1_end, line_pos_y+half_dom_height, fill=colorDict[dom1_name], outline=outliner(colorDict[dom1_name]))
					draw.polygon([dom1_end-oL80, line_pos_y+half_dom_height, dom1_start, line_pos_y, dom1_end-oL80, line_pos_y-half_dom_height,dom1_end, line_pos_y-half_dom_height, dom1_end, line_pos_y+half_dom_height], fill=colorDict[dom1_name], outline=outliner(colorDict[dom1_name]))

				textd1 = canvas.create_text(dom1_start+(dom1_len/2),line_pos_y, text=operonFamily(dom1_name), font=myFont7)
				draw.text((dom1_start+(dom1_len/2),line_pos_y), str(operonFamily(dom1_name)), font=myIMFont7)

				coln=coln+1
	image_test = args.out_prefix+"_IM_treeOrder_flankgenes.eps"
	image1.save(image_test,dpi=300)
